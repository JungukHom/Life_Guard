...
public class TeleportVive : MonoBehaviour {
    ...
    IEnumerator CheckTriggerBox()
    {
        ...
        while (true)
        {
            ...
            {
                if (hit.collider.tag == "TriggerBox")
                {
                    hit.collider.transform.GetComponent<EventTest>().TriggerEnter();
                    break;
				}
				else if(hit.collider.name == "OutPoint")
				{
                    hit.collider.transform.GetComponent<OutPointBox>().TriggerEnter();
                }
            }
    ...
    void Start()
    {
        StartCoroutine("CheckTriggerBox");
	...
	void Update ()
    {
        ...
        else //CurrentTeleportState == TeleportState.None
        {
            ...
            // if necessary.
            SteamVR_Behaviour_Pose obj = Controllers[0];
